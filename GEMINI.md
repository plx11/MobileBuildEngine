...
- **代碼品質規範**: 嚴禁使用「簡化示意」、「架構示意」或 `TODO` 註解來跳過核心實作。所有功能必須提供 100% 完整、經過測試且具備正確錯誤處理的生產級程式碼。
- **錯誤處理**: 所有調用外部二進位的過程必須處理 `ProcessBuilder` 錯誤流與異常，並嚴格檢查進程退出代碼 (`exitValue() == 0`)。
- **權限管理**: `ToolchainManager` 確保執行權限。
- **依賴規範**: 優先使用 `MavenDependencyManager`，且必須實現完整的遞迴解析邏輯。
